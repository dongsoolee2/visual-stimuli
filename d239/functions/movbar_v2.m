...
%
% by Dongsoo Lee (created 17-09-25, edited 17-09-25, v2_edited:17-10-10
%                                                              17-10-18)
...
try
    ...
    bar(2, :) = 0; %yOffset;                        % upper border
    bar(3, :) = xOffset + barWidth;                 % right border
    bar(4, :) = ySize; %yOffset + stimSize;         % bottom border
    ...
    for ind = 1:NUM_OF_BARS
        if barSpeed(ind) > 0
            movingbar_temp = bar - [barWidth; 0; barWidth; 0];
        ...
        elseif barSpeed(ind) < 0
            movingbar_temp = bar + [stimSize; 0; stimSize; 0];
            while movingbar_temp(1, end) > xOffset
                movingbar_temp = [movingbar_temp movingbar_temp(:, end) + [barSpeed(ind) 0 barSpeed(ind) 0]'];
            end
            movingbar{ind} = floor(movingbar_temp);
        end
        ...
        for i = 1:NUM_OF_BARS
            for j = 1:size(movingbar{i}, 2)
                if j == 1                               % first
                    ...
                    Screen('DrawingFinished', myWindow);
                    vbl = Screen('Flip', myWindow, vbl + (WAITFRAMES - 0.5) * ifi);
                elseif j == size(movingbar{i}, 2)       % the last 
                    Screen('FillRect', myWindow, barIntensity(i), movingbar{i}(:, j));
                    Screen('FillOval', myWindow, black, PHOTODIODE);
                    Screen('DrawingFinished', myWindow);
                    vbl = Screen('Flip', myWindow, vbl + (WAITFRAMES - 0.5) * ifi);
