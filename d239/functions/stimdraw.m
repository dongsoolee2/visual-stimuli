...
config = loadjson('/home/dlee/Documents/MATLAB/visual-stimuli/d239/config.json');
...
try
    ...
    for s = 1:stimnum
        ...
        so{s}.Y1 = floor((0:sl{s}.numBoxes - 1)/sl{s}.numHBoxes) * 1 * sl{s}.boxSize + sl{s}.yOffset;
        so{s}.Y2 = so{s}.Y1 + 1 * sl{s}.boxSize;
        ...
        so{s}.pdColor = uint8(repelem(0.00 * so{s}.pdColor, 1, 2));
        % first frame pd
    ...
    if expmode == 1
        ...
        savejson('obj', ex, 'filename', '/home/dlee/Documents/MATLAB/visual-stimuli/d239/archive/' + string(dt) + '.json');
    end
catch exception
    Priority(0);
    Screen('CloseAll');
