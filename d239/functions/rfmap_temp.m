...
try
    ...
    rng(seed);          % default = 0;
    for c = 1:totalFrame
        boxColor(1, :, c) = (rand(1, numColumns) > 0.5) * 2 * (meanIntensity * contrast) ...
            + meanIntensity * (1 - contrast);
        %boxColor(1, :, c) = (rand(s1, 1, numColumns) > 0.5) * 2 * (meanIntensity * contrast) ...
        %    + MeanIntensity * (1 - contrast);
        boxColor(2, :, c) = boxColor(1, :, c);
        boxColor(3, :, c) = boxColor(1, :, c);
    end
    ...
    for i = 1:totalFrame + 1
        if i == 1
            Screen('FillRect', myWindow, boxColor(1, 1, i), boxes);
            Screen('FillOval', myWindow, white, PHOTODIODE);
            vbl = Screen('Flip', myWindow, vbl + (flipFrame - 0.5) * ifi);
        elseif i == totalFrame + 1
            Screen('FillRect', myWindow, boxColor(1, 1, i - 1), boxes);
            Screen('FillOval', myWindow, white, PHOTODIODE);
            vbl = Screen('Flip', myWindow, vbl + (flipFrame - 0.5) * ifi);
        else
            Screen('FillRect', myWindow, boxColor(1, 1, i), boxes);
            Screen('FillOval', myWindow, 0.5 * boxColor(1, 1, i), PHOTODIODE);
            vbl = Screen('Flip', myWindow, vbl + (flipFrame - 0.5) * ifi);
        end
        % keyboard check
        if KbQueueCheck(-1);
